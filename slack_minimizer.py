...
class SlackMinimizer(object):
    ...
    def __init__(self, b_vector, unlabeled_set, holdout_set, holdout_labels, 
                 unlabeled_labels=None, validation_set=None, validation_labels=None, 
                 weights=None, soft_threshold_epsilon=0.0):
        ...
        # For soft thresholding ([weighted] L1 regularization), can be a scalar or a vector
        self._soft_thresh_eps = soft_threshold_epsilon
    ...
    def predict(self, dataset, binary_preds=False):
        """
        ...
        Args:
            dataset: Matrix of specialist predictions on data. Dimensions: {# examples} rows, 
                {# classifiers} columns. E.g. the matrix generated by CompositeFeature.predictions(...). 
            binary_preds: Optional Boolean, specifies whether the predictions are randomized in {-1,1}.
        Returns:
            A vector representing the muffled predictions on the dataset, using 
            the current weight vector (self.weights).
        """
        preds = np.clip(dataset.dot(self.weights), -1, 1)
        # preds = 2.0*scipy.special.expit(dataset.dot(self.weights)) - 1
        if not binary_preds:
            return preds
        else:
            return 2.0 * np.random.binomial(1, 0.5 * (1.0+preds)) - 1.0
    ...
    def sgd(self, duration, linesearch=True, linesearch_tol=0.00005,
        ...
        for iterct in range(duration):
            ...
            # TODO: Can also add an option to use sp.optimize.brent instead of our custom golden section search implementation.
            if linesearch:
                max_stepsize = 2*EMA_stepsize      # Increase each iteration to allow the allowed step size to increase.
            ...
            if np.max(self._soft_thresh_eps) == 0.0:
                self.weights = np.maximum(self.weights, 0.0)   # Project to positive orthant
            else:      # if we're using L1 regularization instead, do soft thresholding.
                wttmp = np.maximum(np.abs(self.weights) - self._soft_thresh_eps, 0.0)
                self.weights = np.multiply(wttmp, np.sign(self.weights))
            self._scoresunl = self.unlabeled_set.dot(self.weights)
    ...
    def _hallucinate_labels(self, scores=None):
        ...
        which default to current unlabeled scores. Treats borderline labels as clipped, 
...
if __name__ == "__main__":
    ...
    with open(logname_auc, 'a') as fo_auc, open(logname_err, 'a') as fo_err:
        ...
        for mctrial in range(args.num_MCtrials):
            ...
            print 'Random forest AUC:\t ' + str(sklearn.metrics.roc_auc_score(
                y_validate, -1.0 + 2*rf.predict_proba(x_validate)[:, 1]))
            print len(b_vector), np.sum(gradh.weights)
